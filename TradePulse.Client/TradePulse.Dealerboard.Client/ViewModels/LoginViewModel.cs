...
public partial class LoginViewModel : ObservableObject
{
    ...
    private async void Login()
    {
        ...
        catch (Exception ex)
        {
            ...
            if (errorMsg.Contains("SSL") || errorMsg.Contains("certificate") || errorMsg.Contains("TLS") || errorMsg.Contains("corrupted frame"))
            {
                ErrorMessage = "SSL error: the server URL is probably wrong.\n\nUse HTTP on port 5000, for example:\nhttp://192.168.1.58:5000\n\nPort 3000 is the web UI only. Do not use https:// unless the server has HTTPS enabled.";
            }
