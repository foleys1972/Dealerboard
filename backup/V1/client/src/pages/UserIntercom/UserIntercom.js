...
const UserIntercom = () => {
  ...
  const computePresence = useCallback(() => {
    if (!socket || !socket.connected) return { key: 'offline', label: 'Logged out' };
                  ...
                  <BroadcastHeader>
                    <BroadcastName>
                      <BroadcastTitleRow>
                        ...
                        <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{broadcast.name}</span>
                        <MonitorToggle
                ...
                >
                  <ContactInfo>
                    <ContactName style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{group.name}</span>
...
const QuickActionButton = styled.button`
  ...
  backdrop-filter: blur(10px);
  border: 2px solid ${({ $active, $color }) => ($active ? $color : 'rgba(255, 255, 255, 0.3)')};
  color: white;
  height: 36px;
  padding: 0 0.85rem;
  border-radius: 8px;
...
const BroadcastItem = styled.div`
  background: ${({ $active, $bgColor }) => ($active ? '#f0fdf4' : ($bgColor || '#f9fafb'))};
  border: 2px solid ${({ $active }) => ($active ? '#4ade80' : '#e5e7eb')};
  border-radius: 12px;
  padding: 1rem;
  min-height: 72px;
  width: 70%;
...
const ContactItem = styled.div`
  ...
  gap: 1rem;
  padding: 1rem;
  background: ${({ $bgColor }) => ($bgColor || '#f9fafb')};
  ...
  &:hover {
    background: ${props => props.disabled ? '#f9fafb' : '#f0fdf4'};
