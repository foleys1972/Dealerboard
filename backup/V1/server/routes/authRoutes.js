...
const traderPasswordHash = bcrypt.hashSync('REDACTED_ROTATED_SECRET', 10);
...
router.post('/register', authenticateToken, async (req, res) => {
  try {
    ...
    const { username, email, firstName, lastName, password, role = 'trader' } = req.body;
...
router.put('/users/:userId', authenticateToken, async (req, res) => {
  try {
    ...
    // Update in database
    const updatedRecord = await updateUser(userId, {
      username,
      firstName,
      lastName,
      email,
