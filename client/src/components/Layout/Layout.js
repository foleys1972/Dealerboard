...
const LogoWithStatus = styled.div`
  display: flex;
  align-items: center;
  gap: ${props => props.theme.spacing.sm};
  flex: 1;
...
const TopBarStatusDot = styled.div`
  width: 8px;
  height: 8px;
  border-radius: 50%;
  background: ${props => props.$isOnline ? '#10b981' : '#ef4444'};
        ...
        <TopBar>
          <TopBarLeft>
            ...
            <ConnectionStatus 
              ...
              title={socketConnected ? 'WebSocket connected' : 'WebSocket disconnected - Click to retry connection'}
              onClick={() => !socketConnected && reconnectSocket()}
              style={{ cursor: socketConnected ? 'default' : 'pointer' }}
            >
              <TopBarStatusDot $isOnline={socketConnected} />
