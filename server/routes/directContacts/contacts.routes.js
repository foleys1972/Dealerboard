...
router.get('/', async (req, res) => {
  try {
    const ownerId =
      (req.user.role === 'platform_admin' || req.user.role === 'tenant_admin') && req.query.ownerId
        ? req.query.ownerId
        : req.user.id;
...
router.post('/', async (req, res) => {
  try {
    ...
    const targetOwnerId =
      (req.user.role === 'platform_admin' || req.user.role === 'tenant_admin') && requestedOwnerId
        ? requestedOwnerId
        : req.user.id;
...
router.delete('/:contactId', async (req, res) => {
  try {
    ...
    let requesterDbId = req.user.id;
    try {
      const requester = await getUserByIdOrUsername(req.user.id);
      if (requester?.id) requesterDbId = requester.id;
    } catch {}

    if (
      contact.ownerId !== requesterDbId &&
